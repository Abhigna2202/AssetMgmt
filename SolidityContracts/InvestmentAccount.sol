...
contract InvestmentAccount{
    ...
    function deductFromAssets(string memory _shareName, uint _deductVal) public payable{
        if (_deductVal >= 0){
            uint temp = AssetPortfolio[_shareName];
    ...
    function sellAssets(string memory _shareName, uint _deductVal, uint256 amount) public payable{
        if (_deductVal >= 0){
            uint temp = AssetPortfolio[_shareName];
            temp = temp - _deductVal;
            AssetPortfolio[_shareName] = temp;
